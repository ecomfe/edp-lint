...
function report(errors) {
    ...
    for (var type in errors) {
        if (errors.hasOwnProperty(type) && errors[type].length) {
            reporter.report(errors[type], edp.log.info, edp.log.warn);
            t12 = false;
        }
    }

    if (t12) {
        edp.log.info('Congratulations! Everything is OK, you must be the T12!');
    }
    else {
        process.exit(1);
    }
 ...
 * @param {Object} options 命令行配置项对象
 */
exports.check = function (candidates, checkers, options) {
    var errors = {};
    checkers.forEach(function (checker) {
        errors[checker.type] = [];
    });

    options.lookup = require('./util').toBoolean(options.lookup, true);
    ...
    function deal(candidate) {
        ...
        checkers.forEach(function (checker) {
            ...
            if (~checker.extensions.indexOf(extname)) {
                checker.check(file, errors[checker.type], options, callback);
            }
        });
    }
