/**
 * Clone from https://github.com/roarkely/htmllint
 * Modifier: wfsr@foxmail.com
 *
 * @file htmlhint 模块
...
exports.lint = function Lint(src, rules) {
    var info = [];
    ...
    var getLocation = function (offset) {
        var lines = src.slice(0, offset).split('\n');
    ...
    var lint = {
        ...
        warn: function (m) {
            ...
            var attr = onValue ? onValue : (m.attribute || null);
            var message = _.isString(m) ? m : (m.warn || 'ERROR');
            var offset = _.isString(m) ? 0 : (m.offset || 0);
            var match = _.isString(m) ? null : (m.match.join('') || null);
        ...
        parser_warn: function (msg, match, offset) {
            if (msg === 'TAG_SELF_CLOSED' || msg === 'TAG_TRAILING_SPACE') {
                return;
            }

            // ignore ENCODE_* warnings from script or style
            if (~msg.indexOf('ENCODE_')) {
                var node = cxt;
                while ((node = node.parentNode)) {
                    if (~'SCRIPT,STYLE'.indexOf(node.nodeName)) {
                        return;
                    }
                }
            }

            match = _.flatten(match).join('');

            var loc = getLocation(offset);

            info.push({
                message: msg + (match ? ': ' + match : ''),
        ...
        parser_emit: function (msg, match, offset) {
            ...
            case 'DOCTYPE':
                // Set doctype to doc
                if (value.toLowerCase() !== '<!doctype html>') {
            ...
            case 'ATTRIBUTE_NAME':
                ...
                var tag = cxt.tagName.toLowerCase();

                if (stack.length > 1 && stack[stack.length - 2] === 'script') {
                    stack.pop();
                    cxt = cxt.parentNode;
                    break;
                }
                ...
                if (!(/^data-.+$/.test(attr)
                    || _.where(rules, { element: '*' })[0].attributes[attr]
                    || _.where(rules, { element: tag })[0].attributes[attr])
                ) {
                    lint.parser_warn(
            ...
            case 'ATTRIBUTE_VALUE':
                ...
                if (last_attr !== null) {
                    cxt.setAttribute(last_attr, value);
                    if (cxt.attr_offset[last_attr]) {
                        cxt.attr_offset[last_attr].value = offset;
                    }

                    if (last_attr === 'id' && value in ids) {
                        lint.parser_warn('DUPLICATE_ID', [value], offset);
                    }
                    else {
                        ids[value] = 1;
                    }
            ...
            case 'EOF':
                _.each(stack, function (value) {
                    lint.parser_warn('UNCLOSED_TAG', [value], offset);
                });
                break;
