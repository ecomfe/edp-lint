...
var Checker = require('edp-jscs/lib/checker');
...
function jshintTask(file) {
    return function(callback) {
        ...
        var jshintConfig = util.getConfig('.jshintrc', file.path,
...
function getProjectJSCSConfig() {
    ...
    if (!fs.existsSync(path.join(bizConfigPath, '.jscsrc'))) {
    ...
    if (bizConfigPath) {
        bizConfig = JSON.parse(
            fs.readFileSync(
                path.join(bizConfigPath, '.jscsrc'),
                'utf-8'
            )
        );
    }
